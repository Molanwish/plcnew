...
class DataSavedEvent(DataEvent):
    ...
    def __init__(self, data_type: str, data_id: str, filepath: str):
...
class DataErrorEvent(DataEvent):
    ...
    Attributes:
        data_type (str): 数据类型
        operation (str): 操作类型(save/load/query)
        error (str): 错误消息
    """
...
class CycleActionRequestEvent(Event):
    ...
    VALID_ACTIONS = ["refresh", "export_selected", "export_all", 
                     "delete_selected", "analyze_selected"]

    def __init__(self, action: str, cycle_id: str = None):
        super().__init__()
        assert action in self.VALID_ACTIONS, f"Invalid cycle action: {action}"
        self.action = action
        self.cycle_id = cycle_id
...
class ChartTypeRequestEvent(Event):
    ...
    def __init__(self, chart_type: str):
        super().__init__()
        # 实际应用中可能需要更严格的验证，确保类型存在于UI控件中
        if chart_type not in self.VALID_CHART_TYPES:
             print(f"Warning: Requested chart type '{chart_type}' might not be supported.")
...
class GlobalCommandRequestEvent(Event):
    ...
    def __init__(self, command: str):
        ...
        if command not in self.VALID_COMMANDS:
            print(f"Warning: Requested global command '{command}' might not be standard.")
